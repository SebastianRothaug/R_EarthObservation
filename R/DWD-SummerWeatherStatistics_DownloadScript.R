...
## necessary if we would have other climate parametres just as precipitation in it aswell
...
rasterHist <- rasterHist/10 ## Temperatur in zehntel Grad Celsius 
...
pdf("August_mean_vs_2018.pdf", width = 14, height = 8) ## leeres!! nicht offenes PDF erstellt
grid.arrange(p1, p2, ncol=2)                          ## PDF wird mit den plots, p1, p2 gefüllt
dev.off()                                             ## fertiges PDF
...
library(geodata)
bnd <- geodata::gadm("GDAM",country = "DEU", level=1)
plot(bnd)

### terra Package Flo
##bnd.utm <- spTransform(bnd, CRS(proj4string(my_raster)))
bnd.utm <- terra::project(bnd, crs(my_raster))
